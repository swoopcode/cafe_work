...
jQuery(function(){
	...
	jQuery('#daumSearch').on('submit', function(e) {
		e.preventDefault();
		if (!this.isSearchMode) { // 珥덇린 寃��됱갹�� �대━吏� �딆� 耳��댁뒪
			this.isSearchMode = true;
			jQuery('#daumSearch').find('.search_choice').addClass('search_open');
			jQuery('#daumSearch').find('.ico_search').text('寃��됲븯湲�');
			return;
		}
	...
	if(getCookie("M_P2M") && checkMobileUserAgent()){
		var redirectMobileBtn = jQuery("<div id='redirect_mobile'><a href=\"https://m.cafe.daum.net\" onclick=\"deleteCookie(\'M_P2M\',\'.daum.net\');\" class=\"btn_go_mobile\"><span>紐⑤컮�쇳솕硫� �대룞</span></a></div>");
